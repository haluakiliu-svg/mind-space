...
async function createPost(post) {
  ...
  // If .select().single() fails (e.g. anon has no SELECT), try insert-only
  if (error) {
    console.warn('createPost with select failed, trying insert-only:', error.message);
    const { error: err2 } = await db.from('posts').insert([post]);
    if (err2) { console.error('createPost error:', err2); return null; }
    // Return a mock object with timestamp so UI can display immediately
    return { ...post, id: 'local-' + Date.now(), created_at: new Date().toISOString() };
  }
  return data;
}
